...
async function fileHash(file){ try{ const buf=await file.arrayBuffer(); const d=await crypto.subtle.digest("SHA-256", buf); return [...new Uint8Array(d)].map(b=>b.toString(16).padStart(2,"0")).join(""); }catch(e){ return null; } }
function hashPath(file, hash){ const n=(file&&file.name)||"file"; let ext=n.indexOf(".")>-1?n.split(".").pop().toLowerCase():"pdf"; ext=ext.replace(/[^a-z0-9]/g,"").slice(0,5)||"pdf"; return hash+"."+ext; }
...
function BulkUpload({ onBack, onSubmit, existingPaths }) {
  ...
  const handleFiles = async (files) => {
    ...
    (async()=>{const ex=new Set((existingPaths||[]).map(p=>String(p).split("/").pop().split(".")[0]));const seen=new Set();for(const it of items){const hsh=await fileHash(it.file);it.__hash=hsh;it.__dup=hsh?(ex.has(hsh)||seen.has(hsh)):false;if(hsh)seen.add(hsh);}setQueue(q=>q.map(x=>{const m=items.find(i=>i.id===x.id);return m?{...x,__hash:m.__hash,__dup:m.__dup}:x;}));})();
    setQueue(items);
...
export default function App() {
  ...
  const submitInvoice=async()=>{const e=validate();setErr(e);if(Object.keys(e).length)return;let pdf_path=null;if(dupWarn){if(!window.confirm("This exact file was already uploaded as "+dupWarn.supplier+" "+(dupWarn.invoice_no||dupWarn.invoiceNo||"")+". Submit it again anyway?"))return;}if(pendingFile){const hsh=pendingHash||await fileHash(pendingFile);const path=hsh?hashPath(pendingFile,hsh):(crypto.randomUUID?crypto.randomUUID():Date.now()+"-"+Math.random().toString(36).slice(2))+"/"+(pendingFile.name||"file.pdf").replace(/[^\w.\-]+/g,"_");const up=await supabase.storage.from("invoices").upload(path,pendingFile,{contentType:pendingFile.type||"application/pdf",upsert:true});if(!up.error)pdf_path=path;}const res=await supabase.from("invoices").insert([{supplier:form.supplier,invoice_no:form.invoiceNo,invoice_date:form.invoiceDate,due_date:form.dueDate,amount:parseFloat(form.amount),currency:form.currency,department:form.department,equipment:form.equipment||null,job_ref:form.jobRef||null,notes:form.notes||null,file_name:form.fileName||null,pdf_path:pdf_path,budget_id:form.budgetId||null,status:"Pending",created_by:userName}]).select();if(res.data&&res.data[0]){setInv(p=>[res.data[0],...p]);push("Captain","New invoice from "+form.supplier+" awaiting approval");setSub(true);}};

  const resetForm=()=>{setForm(emptyForm);setErr({});setSub(false);setLang(null);setPendingFile(null);setPendingHash(null);setDupWarn(null);};

  const submitBulk=async(done)=>{const inserts=[];for(const x of done){let pdf_path=null;if(x.file){const _h=x.__hash||await fileHash(x.file);const path=_h?hashPath(x.file,_h):(crypto.randomUUID?crypto.randomUUID():Date.now()+"-"+Math.random().toString(36).slice(2))+"/"+((x.file.name||"file.pdf").replace(/[^\w.\-]+/g,"_"));const up=await supabase.storage.from("invoices").upload(path,x.file,{contentType:x.file.type||"application/pdf",upsert:true});if(!up.error)pdf_path=path;}inserts.push({supplier:x.data.supplier,invoice_no:x.data.invoiceNo,invoice_date:x.data.invoiceDate||null,due_date:x.data.dueDate||null,amount:parseFloat(x.data.amount)||0,currency:x.data.currency,department:x.data.department,equipment:x.data.equipment||null,job_ref:x.data.jobRef||null,notes:x.data.notes||null,file_name:x.data.fileName||null,pdf_path:pdf_path,budget_id:x.data.budgetId||null,status:"Pending",created_by:userName});}const res=await supabase.from("invoices").insert(inserts).select();if(res.data)setInv(p=>[...res.data,...p]);push("Captain",done.length+" new invoice"+(done.length!==1?"s":"")+" awaiting approval");setBulk(false);setView("tracker");};
  ...
  return (
    <div style={{fontFamily:"'Inter','Segoe UI',Arial,sans-serif",background:"#f4f6f8",minHeight:"100vh"}}>
      ...
      <div style={{padding:24}}>
        ...
        {page==="invoices"&&bulk&&<BulkUpload onBack={()=>setBulk(false)} onSubmit={submitBulk} existingPaths={invoices.map(i=>i.pdf_path).filter(Boolean)}/>}
        ...
        {page==="invoices"&&!bulk&&view==="upload"&&!submitted&&<div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,maxWidth:860}}>
            <div style={{...SS.card,padding:20}}>
              ...
              {lang&&<div style={{marginTop:12,padding:"10px 14px",background:"#E8F5E9",borderRadius:4,border:"1px solid #A5D6A7",fontSize:13,color:"#2E7D32"}}>✨ Extracted from {lang} {lF[lang]||""} — review fields</div>}{dupWarn&&<div style={{marginTop:12,padding:"10px 14px",background:"#FFF8E1",borderRadius:4,border:"1px solid #FFE082",fontSize:13,color:"#E65100"}}>⚠️ This exact file was already uploaded — {dupWarn.supplier} · {dupWarn.invoice_no||dupWarn.invoiceNo} ({dupWarn.status}). You can still submit if intentional.</div>}
            </div>
            <div style={{...SS.card,padding:20}}>
