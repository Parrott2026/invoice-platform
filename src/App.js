...
function downloadInvoicePDF(inv, budgets) {
  ...
  const html = `<html><head><style>body{font-family:Arial,sans-serif;padding:40px;color:#222;max-width:700px;margin:0 auto;}h1{color:#1AAFBF;}table{width:100%;border-collapse:collapse;margin-bottom:24px;}td{padding:10px 12px;border-bottom:1px solid #f0f0f0;font-size:14px;}td:first-child{color:#666;width:180px;font-weight:500;}.badge{display:inline-block;padding:4px 12px;border-radius:4px;font-size:13px;font-weight:600;background:#E8F5E9;color:#2E7D32;}.sage{background:#f7f9fc;border:1px solid #dde1e7;border-radius:6px;padding:16px 20px;margin-top:24px;}.footer{margin-top:40px;padding-top:16px;border-top:1px solid #eee;font-size:12px;color:#aaa;text-align:center;}</style></head><body>
    <h1>Invoice Record</h1>
    <p style="color:#888;font-size:13px;margin:0 0 32px">Generated ${new Date().toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}</p>
    <table>
      <tr><td>Supplier</td><td><strong>${inv.supplier}</strong></td></tr>
      <tr><td>Invoice No.</td><td>${inv.invoice_no||inv.invoiceNo||""}</td></tr>
      <tr><td>Invoice date</td><td>${inv.invoice_date||inv.invoiceDate||""}</td></tr>
      <tr><td>Due date</td><td>${inv.due_date||inv.dueDate||""}</td></tr>
      <tr><td>Amount</td><td><strong>${inv.currency} ${fmt(inv.amount)}</strong></td></tr>
      <tr><td>Department</td><td>${inv.department||""}</td></tr>
      ${inv.equipment ? `<tr><td>Equipment</td><td>${inv.equipment}</td></tr>` : ""}
      ${inv.job_ref||inv.jobRef ? `<tr><td>Job ref</td><td>${inv.job_ref||inv.jobRef}</td></tr>` : ""}
      ${budget ? `<tr><td>Budget</td><td>${budget.name}</td></tr>` : ""}
      ${inv.notes ? `<tr><td>Notes</td><td>${inv.notes}</td></tr>` : ""}
      <tr><td>Status</td><td><span class="badge">Approved</span></td></tr>
    </table>
    <div class="sage"><h3 style="margin:0 0 12px;font-size:14px;color:#444;">Sage import reference</h3>
    <table>
      <tr><td style="color:#666;width:160px">Supplier</td><td>${inv.supplier}</td></tr>
      <tr><td style="color:#666">Invoice ref</td><td>${inv.invoice_no||inv.invoiceNo||""}</td></tr>
      <tr><td style="color:#666">Invoice date</td><td>${inv.invoice_date||inv.invoiceDate||""}</td></tr>
      <tr><td style="color:#666">Due date</td><td>${inv.due_date||inv.dueDate||""}</td></tr>
      <tr><td style="color:#666">Net amount</td><td>${inv.currency} ${fmt(inv.amount)}</td></tr>
      <tr><td style="color:#666">Nominal code</td><td>${inv.department||""}</td></tr>
    </table></div>
    <div class="footer">Invoice Platform · Approved by Captain · ${new Date().toLocaleDateString("en-GB")}</div>
  </body></html>`;
      ...
      {!selected && (
        <div style={{background:"#fff",border:"1px solid #dde1e7",borderRadius:8,padding:"32px 40px",width:"100%",maxWidth:380}}>
          ...
          {[{key:"ce",icon:"🔧",label:"Chief Engineer",sub:"Upload & manage invoices"},{key:"captain",icon:"⚓",label:"Captain",sub:"Review & approve invoices"}].map(u => (
            <button key={u.key} onClick={() => { setSelected(u.key); setPin(""); setError(""); }}
              style={{width:"100%",padding:"16px 20px",fontSize:15,fontWeight:500,background:"#fff",color:"#222",border:"2px solid #dde1e7",borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",gap:14,textAlign:"left",marginBottom:12}}>
      ...
      {selected && (
        <div style={{background:"#fff",border:"1px solid #dde1e7",borderRadius:8,padding:"32px 40px",width:"100%",maxWidth:340,textAlign:"center"}}>
          ...
          <div style={{position:"relative",marginBottom:16}}>
            <input type={showPin?"text":"password"} value={pin} maxLength={4} placeholder="4-digit PIN" autoFocus
              onChange={e => { setPin(e.target.value.replace(/\D/g,"").slice(0,4)); setError(""); }}
              onKeyDown={e => { if(e.key==="Enter") tryLogin(); }}
              style={{...SS.inp,textAlign:"center",fontSize:20,letterSpacing:8,borderColor:error?"#C62828":"#ccc",paddingRight:40}}/>
...
function DropZone({ onFile, fileName, extracting }) {
  ...
  return (
    <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
      ...
      onClick={()=>{if(!extracting)ref.current.click();}}
      style={{border:"2px dashed "+(drag?TEAL:"#ccc"),borderRadius:6,padding:"32px 20px",cursor:extracting?"default":"pointer",textAlign:"center",background:drag?TEAL_LIGHT:"#fafafa"}}>
  ...
  return (
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
      <thead><tr style={{background:"#f7f8fa"}}>
        {["Supplier","Invoice No.","Department","Amount","Status","Actions"].map(h => (
          <th key={h} style={{padding:"11px 16px",textAlign:"left",fontWeight:600,color:"#555",fontSize:13,borderBottom:"2px solid #e0e0e0",whiteSpace:"nowrap"}}>{h}</th>
        ))}
      </tr></thead>
      <tbody>
            ...
            <td style={{padding:"10px 16px"}}>
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {showActions && inv.status==="Pending" && <>
                  <button style={SS.btnTs} onClick={()=>onStatus(inv.id,"Approved")}>✓ Approve</button>
                  <button style={{...SS.btnGs,color:"#C62828",borderColor:"#EF9A9A"}} onClick={()=>onStatus(inv.id,"Rejected")}>✕ Reject</button>
                </>}
                {showActions && inv.status!=="Pending" && <button style={SS.btnGs} onClick={()=>onStatus(inv.id,"Pending")}>↺ Reset</button>}
                {inv.status==="Approved" && <button onClick={()=>downloadInvoicePDF(inv,budgets)} style={{...SS.btnGs,color:"#2E7D32",borderColor:"#A5D6A7",fontSize:12,padding:"4px 10px"}}>⬇ Download for Sage</button>}
              </div>
            </td>
          </tr>
        ))}
...
function BudgetList({ budgets, invoices, onSelect, onAdd }) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <button style={SS.btnT} onClick={()=>setAddB(v=>!v)}>+ New budget</button>
      </div>
      {addB && (
        <div style={{...SS.card,padding:20,marginBottom:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:14,marginBottom:14}}>
            <div><Lbl required>Name</Lbl><input value={bForm.name} placeholder="e.g. Summer Season" onChange={e=>{const v=e.target.value;setBForm(p=>({...p,name:v}));}} style={{...SS.inp,borderColor:bErr.name?"#C62828":"#ccc"}}/>{bErr.name&&<p style={{fontSize:12,color:"#C62828",margin:"4px 0 0"}}>Required</p>}</div>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button style={SS.btnGs} onClick={()=>{setAddB(false);setBErr({});setBForm(emptyBudget);}}>Cancel</button>
          </div>
        </div>
      )}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        {budgets.map(b => {
          ...
          return (
            <div key={b.id} onClick={()=>onSelect(b.id)} style={{background:"#fff",border:"1px solid #dde1e7",borderRadius:6,padding:"16px 18px",cursor:"pointer",borderTop:"3px solid "+TEAL}}>
    ...
    <div>
      ...
      {byD.length>0 && (
        <div style={{...SS.card,padding:20,marginBottom:16}}>
          ...
          {byD.sort((a,b)=>b.total-a.total).map((d,i) => (
      ...
      {byS.length>0 && (
        <div style={{...SS.card,padding:20,marginBottom:16}}>
          ...
          {byS.sort((a,b)=>b.total-a.total).map((s,i) => (
  ...
  const handleFiles = async (files) => {
    ...
    for (let i=0; i<items.length; i++) {
      ...
      try {
        ...
        const errId = cur.id;
        setQueue(q => q.map(x => x.id===errId ? {...x, status:"error"} : x));
      }
    }
  };
    ...
    <div>
      ...
      {queue.length===0 && (
        <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
          ...
          <p style={{fontSize:13,color:"#888",margin:0}}>Hold Ctrl or Cmd to select multiple</p>
        </div>
      )}
      {queue.length>0 && (
        <div>
          <div style={{...SS.card,padding:"14px 20px",marginBottom:16,display:"flex",alignItems:"center",gap:16}}>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:6}}>
                <span style={{color:"#555"}}>{extr>0 ? `Extracting ${done.length} of ${queue.length}...` : `${done.length} of ${queue.length} extracted`}</span>
            ...
            <button style={SS.btnGs} onClick={()=>setQueue([])}>Clear all</button>
          </div>
          <div style={{...SS.card,marginBottom:16}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr style={{background:"#f7f8fa"}}>
                {["File","Supplier","Invoice No.","Date","Amount","Cur.","Status",""].map(h => <th key={h} style={{padding:"10px 12px",textAlign:"left",fontWeight:600,color:"#555",fontSize:12,borderBottom:"2px solid #e0e0e0",whiteSpace:"nowrap"}}>{h}</th>)}
              </tr></thead>
              <tbody>
                {queue.map((item,i) => (
                  <tr key={item.id} style={{borderBottom:"1px solid #f0f0f0",background:i%2===0?"#fff":"#fafbfc"}}>
                    ...
                    <td style={{padding:"6px 8px"}}>{item.status==="done" ? <input value={item.data.supplier} onChange={e=>{const v=e.target.value;updateField(item.id,"supplier",v);}} style={{...SS.inp,fontSize:13,padding:"4px 8px"}} placeholder="Supplier"/> : <span style={{color:"#aaa",fontSize:12}}>{item.status==="extracting"?"Extracting...":"Queued"}</span>}</td>
                    <td style={{padding:"6px 8px"}}>{item.status==="done" ? <input value={item.data.invoiceNo} onChange={e=>{const v=e.target.value;updateField(item.id,"invoiceNo",v);}} style={{...SS.inp,fontSize:13,padding:"4px 8px"}} placeholder="INV-001"/> : <span style={{color:"#aaa",fontSize:12}}>—</span>}</td>
                    <td style={{padding:"6px 8px"}}>{item.status==="done" ? <input type="date" value={item.data.invoiceDate} onChange={e=>{const d=e.target.value;let due="";if(d){const dt=new Date(d);dt.setDate(dt.getDate()+30);due=dt.toISOString().split("T")[0];}updateField(item.id,"invoiceDate",d);updateField(item.id,"dueDate",due);}} style={{...SS.inp,fontSize:13,padding:"4px 8px"}}/> : <span style={{color:"#aaa",fontSize:12}}>—</span>}</td>
                    <td style={{padding:"6px 8px"}}>{item.status==="done" ? <input type="number" value={item.data.amount} onChange={e=>{const v=e.target.value;updateField(item.id,"amount",v);}} style={{...SS.inp,fontSize:13,padding:"4px 8px",width:80}} placeholder="0.00"/> : <span style={{color:"#aaa",fontSize:12}}>—</span>}</td>
                    <td style={{padding:"6px 8px"}}>{item.status==="done" ? <select value={item.data.currency} onChange={e=>{const v=e.target.value;updateField(item.id,"currency",v);}} style={{...SS.inp,fontSize:13,padding:"4px 6px",width:72}}>{CURRENCIES.map(c=><option key={c}>{c}</option>)}</select> : <span style={{color:"#aaa",fontSize:12}}>—</span>}</td>
                    ...
                    <td style={{padding:"6px 8px"}}><button onClick={()=>removeItem(item.id)} style={{...SS.btnGs,padding:"3px 8px",fontSize:12,color:"#C62828",borderColor:"#EF9A9A"}}>✕</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          {allDone && (
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <button style={SS.btnT} onClick={()=>onSubmit(done)}>Submit {done.length} invoice{done.length!==1?"s":""}</button>
...
export default function App() {
  ...
  useEffect(() => {
    const h = e => { if (nRef.current && !nRef.current.contains(e.target)) setShowN(false); };
  ...
  const loadData = async () => {
    setLoading(true);
    const bRes = await supabase.from("budgets").select("*").order("created_at", {ascending:true});
    const iRes = await supabase.from("invoices").select("*").order("created_at", {ascending:false});
  ...
  const updateStatus = async (id, status) => {
    ...
    setInv(p => p.map(i => i.id===id ? {...i, status} : i));
  ...
  const validate = () => {
    const e = {};
    if (!form.supplier.trim()) e.supplier = true;
    if (!form.invoiceNo.trim()) e.invoiceNo = true;
    if (!form.invoiceDate) e.invoiceDate = true;
    if (!form.dueDate) e.dueDate = true;
    if (!form.amount || isNaN(form.amount) || parseFloat(form.amount)<=0) e.amount = true;
    return e;
  };
  ...
  const submitInvoice = async () => {
    ...
    const res = await supabase.from("invoices").insert([{supplier:form.supplier,invoice_no:form.invoiceNo,invoice_date:form.invoiceDate,due_date:form.dueDate,amount:parseFloat(form.amount),currency:form.currency,department:form.department,equipment:form.equipment||null,job_ref:form.jobRef||null,notes:form.notes||null,file_name:form.fileName||null,budget_id:form.budgetId||null,status:"Pending",created_by:userName}]).select();
    if (res.data && res.data[0]) {
      setInv(p => [res.data[0], ...p]);
  ...
  const F = (field, label, type="text", ph="", req=false) => (
    <div>
      <Lbl required={req}>{label}</Lbl>
      <input type={type} value={form[field]} placeholder={ph} onChange={e=>{const v=e.target.value;setForm(p=>({...p,[field]:v}));}} style={{...SS.inp,borderColor:err[field]?"#C62828":"#ccc"}}/>
      {err[field] && <p style={{fontSize:12,color:"#C62828",margin:"4px 0 0"}}>Required</p>}
    </div>
  );
  ...
  return (
    <div style={{fontFamily:"'Inter','Segoe UI',Arial,sans-serif",background:"#f4f6f8",minHeight:"100vh"}}>
      ...
      <div style={{background:"#fff",borderBottom:"1px solid #dde1e7",padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:54}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{position:"relative"}} ref={nRef}>
            <button onClick={()=>{setShowN(v=>!v);if(!showN)markR();}} style={{...SS.btnGs,position:"relative"}}>
            ...
            {showN && (
              <div style={{position:"absolute",right:0,top:"calc(100% + 6px)",width:300,background:"#fff",border:"1px solid #dde1e7",borderRadius:6,zIndex:300,boxShadow:"0 6px 20px rgba(0,0,0,.1)",overflow:"hidden"}}>
                ...
                {myN.slice().reverse().map(n => <div key={n.id} style={{padding:"11px 16px",borderBottom:"1px solid #f5f5f5",background:n.read?"#fff":"#f0fbfc",fontSize:13,color:"#333",lineHeight:1.5}}>{n.msg}</div>)}
              </div>
            )}
          </div>
      ...
      <div style={{background:NAV_BG,padding:"0 24px",display:"flex"}}>
        {[{key:"invoices",label:"🧾 Invoices"},{key:"budgets",label:"📊 Budgets"}].map(n => (
          <button key={n.key} onClick={()=>{setPage(n.key);setSelB(null);setBulk(false);}}
      ...
      {page==="invoices" && !bulk && (
        <div style={{background:"#fff",borderBottom:"1px solid #dde1e7",padding:"0 24px",display:"flex"}}>
          {(role==="CE" ? ["tracker","upload"] : ["approvals"]).map(v => (
      ...
      <div style={{padding:24}}>
        ...
        {page==="budgets" && !selB && <BudgetList budgets={budgets} invoices={invoices} onSelect={setSelB} onAdd={b=>setBudgets(p=>[...p,b])}/>}
        ...
        {page==="invoices" && !bulk && view==="tracker" && (
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              ...
              {role==="CE" && <div style={{display:"flex",gap:8}}>
                <button style={SS.btnT} onClick={()=>setView("upload")}>+ New invoice</button>
                <button style={SS.btnO} onClick={()=>setBulk(true)}>📂 Bulk upload</button>
              </div>}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:20}}>
              {["Pending","Approved","Rejected"].map((s,i) => {
                const total = invoices.filter(x=>x.status===s).reduce((t,x)=>t+parseFloat(x.amount||0),0);
                return <StatCard key={s} label={s} value={invoices.filter(x=>x.status===s).length} sub={`EUR ${fmt(total)}`} accent={["#FF9800","#4CAF50","#F44336"][i]}/>;
              })}
            </div>
            <div style={SS.card}>
        ...
        {page==="invoices" && !bulk && view==="upload" && !submitted && (
          <div>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
              <button style={SS.btnGs} onClick={()=>setView("tracker")}>← Back</button>
              ...
              <div style={{...SS.card,padding:20}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    ...
                    <div>
                      <Lbl required>Invoice date</Lbl>
                      <input type="date" value={form.invoiceDate} onChange={e=>{const d=e.target.value;let due="";if(d){const dt=new Date(d);dt.setDate(dt.getDate()+30);due=dt.toISOString().split("T")[0];}setForm(p=>({...p,invoiceDate:d,dueDate:due}));}} style={{...SS.inp,borderColor:err.invoiceDate?"#C62828":"#ccc"}}/>
                      {err.invoiceDate && <p style={{fontSize:12,color:"#C62828",margin:"4px 0 0"}}>Required</p>}
                    </div>
                    <div>
                      <Lbl required>Due date <span style={{fontWeight:400,color:"#aaa",fontSize:11}}>auto +30 days</span></Lbl>
                      <input type="date" value={form.dueDate} onChange={e=>{const v=e.target.value;setForm(p=>({...p,dueDate:v}));}} style={{...SS.inp,borderColor:err.dueDate?"#C62828":"#ccc"}}/>
                      {err.dueDate && <p style={{fontSize:12,color:"#C62828",margin:"4px 0 0"}}>Required</p>}
                    </div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"2fr 1fr",gap:12}}>
                    <div>
                      <Lbl required>Amount</Lbl>
                      <input type="number" min="0" step="0.01" value={form.amount} placeholder="0.00" onChange={e=>{const v=e.target.value;setForm(p=>({...p,amount:v}));}} style={{...SS.inp,borderColor:err.amount?"#C62828":"#ccc"}}/>
                      {err.amount && <p style={{fontSize:12,color:"#C62828",margin:"4px 0 0"}}>Required</p>}
                    </div>
        ...
        {page==="invoices" && !bulk && view==="upload" && submitted && (
          <div style={{maxWidth:440,margin:"3rem auto",textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              ...
              <button style={SS.btnO} onClick={()=>{setView("tracker");setSub(false);}}>View tracker</button>
            </div>
          </div>
        )}

        {page==="invoices" && !bulk && view==="approvals" && (
          <div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:24}}>
              ...
              <StatCard label="Approved" value={invoices.filter(i=>i.status==="Approved").length} accent="#4CAF50"/>
              <StatCard label="Rejected" value={invoices.filter(i=>i.status==="Rejected").length} accent="#F44336"/>
            </div>
                ...
                <div key={inv.id} style={{...SS.card,marginBottom:12,borderLeft:"4px solid "+TEAL}}>
                  <div style={{padding:"16px 20px"}}>
                    ...
                    <div style={{display:"flex",gap:8,marginTop:14}}>
                      <button style={SS.btnTs} onClick={()=>updateStatus(inv.id,"Approved")}>✓ Approve</button>
                      <button style={{...SS.btnGs,color:"#C62828",borderColor:"#EF9A9A"}} onClick={()=>updateStatus(inv.id,"Rejected")}>✕ Reject</button>
                    </div>
                  </div>
                </div>
              );
            ...
            {invoices.filter(i=>i.status==="Approved").length>0 && (
              <div>
                ...
                <div style={SS.card}><InvoiceTable invoices={invoices.filter(i=>i.status==="Approved")} budgets={budgets} onStatus={updateStatus} showActions={false}/></div>
              </div>
            )}
            {invoices.filter(i=>i.status==="Rejected").length>0 && (
              <div>
                ...
                <div style={SS.card}><InvoiceTable invoices={invoices.filter(i=>i.status==="Rejected")} budgets={budgets} onStatus={updateStatus} showActions={true}/></div>
              </div>
            )}
          </div>
        )}
